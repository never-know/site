...
LazyLoad.prototype = {
  ...
  _setOptions: function(options) {
    ...
    return Min.obj.extend(this.options, options || {});
  },
  //初始化容器设置
  _initContainer: function(container) {
	...
	this.load = Min.obj.methodReference(this,'_load');
	this.resize = Min.obj.methodReference(this,'_resize');
	...
	//绑定事件
	Min.event.bind( this._binder, "scroll", this.delayLoad );
	isWindow && Min.event.bind( this._binder, "resize", this.delayResize );
	//获取容器位置参数函数
  ...
  _initStatic: function(mode, direction) {
	...
	if ( mode == "cross" ) {
		this._crossDirection = Min.obj.methodReference(this, '_getCrossDirection',[isVertical ? "_verticalDirection" : "_horizontalDirection",isVertical ? "_horizontalDirection" : "_verticalDirection" ]);
	}
	//设置元素
	...
	this._setElems = function() {//转换数组并排序
		this._elems = Array.prototype.map.call(this._elems, getRect, this ).sort( sortFunction );
	};
	//设置加载函数
	this._loadData = Min.obj.methodReference( this,'_loadStatic',[
		"_" + mode + "Direction",
		Min.obj.methodReference( this, '_outofRange', [ mode, "_" + direction + "BeforeRange"] ),
		Min.obj.methodReference( this, '_outofRange', [ mode, "_" + direction + "AfterRange" ])] );
  },
  //延时程序
  _delay: function(run) {
  ...
  _loadDynamic: function() {
	this._elems = this._elems.filter( function( elem ) {
			return !this._insideRange( elem );
		}, this );
  },
  //静态加载程序
  _loadStatic: function(force, direction, beforeRange, afterRange) {
	//获取方向
  ...
  //cross滚动方向获取程序
  _getCrossDirection: function(force,primary, secondary) {
  ...
  //判断是否超过加载范围
  _outofRange: function( middle, elem,mode, compare) {
  ...
  dispose: function(load) {
	...
	if ( this._elems || this._binder ) {
		...
		//清除关联
		Min.event.unbind( this._binder, "scroll", this.delayLoad );
		Min.event.unbind( this._binder, "resize", this.delayResize );
